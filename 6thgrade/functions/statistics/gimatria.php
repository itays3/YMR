...
<?php 
		...
		$username = $_SESSION['username'];
			...
			while($row = mysql_fetch_array($result))
			{
				if ($row['yes_count'] == 1 && $row['subject'] == gimatria1)
				{
					$counter_yes1 = $counter_yes1+1;
				}
				if ($row['no_count'] == 1 && $row['subject'] == gimatria1)
				{
					$counter_no1 = $counter_no1+1;
				}
				if ($row['yes_count'] == 1 && $row['subject'] == gimatria2)
				{
					$counter_yes2 = $counter_yes2+1;
				}
				if ($row['no_count'] == 1 && $row['subject'] == gimatria2)
				{
					$counter_no2 = $counter_no2+1;
				}
				if ($row['yes_count'] == 1 && $row['subject'] == gimatria3)
				{
					$counter_yes3 = $counter_yes3+1;
				}
				if ($row['no_count'] == 1 && $row['subject'] == gimatria3)
				{
					$counter_no3 = $counter_no3+1;
				}
				if ($row['yes_count'] == 1 && $row['subject'] == gimatria4)
				{
					$counter_yes4 = $counter_yes4+1;
				}
				if ($row['no_count'] == 1 && $row['subject'] == gimatria4)
				{
            ...
            <tr><td><p>מאותיות למספרים</p></td><td><P><? if ($total1!=0){ echo intval($counter_yes1/$total1 * 100), "% אחוזי הצלחה"; } 
			else echo "עדיין אין נתונים"?></P></td></tr>
            <tr><td><p>ממספרים לאותיות</p></td><td><P><? if ($total2!=0){ echo intval($counter_yes2/$total2 * 100), "% אחוזי הצלחה"; }
			else echo "עדיין אין נתונים"?></P></td></tr>
            <tr><td><p>סדרות של אותיות</p></td><td><P><?  if ($total3!=0){ echo intval($counter_yes3/$total3 * 100), "% אחוזי הצלחה"; }
			else echo "עדיין אין נתונים"?></P></td></tr>
            <tr><td><p>ערכים של מילים</p></td><td><P><?  if ($total4!=0){ echo intval($counter_yes4/$total4 * 100), "% אחוזי הצלחה"; }
