...
<form method="post" action="plus_d3.php" dir="ltr"> 
...
VALUES
('%s','1','$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_d3','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_d3','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='plus_d3'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET plus_d3='%s' WHERE s_id='%s' ;",
