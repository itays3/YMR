...
<body>
    ...
    <?php 
		...
		?>
    <h1 dir="rtl" style="font-family:Cursive;font-size:36px;color:#3300cc;font-style:italic;font-weight:normal;text-shadow:4px 4px 8px black;">חיבור בעשרות עד 1000</h1>
...
<form method="post" action="plus_c1.php" dir="ltr"> 
    ...
    <!-- this part shows the last exercise and the answer-->
		<tr>
	        ...
	        <?php } ?>
                <td><? include('answer1.php') ?></td></table>
</form>
<?php 
session_start();
...
VALUES
('%s','1','$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[equal]$_POST[answer]','plus_c1','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[equal]$_POST[answer]','plus_c1','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='plus_c1'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET plus_c1='%s' WHERE s_id='%s' ;",
