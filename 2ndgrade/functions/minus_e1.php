...
<h1 dir="rtl" 
    ...
    style="font-family:Cursive;font-size:24px;color:#3300cc;font-style:italic;font-weight:normal;text-shadow:4px 4px 8px black;">פתור את התרגילים הבאים</p>
		...
		?>
<form method="post" action="minus_e1.php" dir="ltr">    
...
VALUES
('%s','1','$_POST[last_x]$_POST[minus]$_POST[last_y]$_POST[equal]$_POST[answer]','minus_e1','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[minus]$_POST[last_y]$_POST[equal]$_POST[answer]','minus_e1','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='minus_e1'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET minus_e1='%s' WHERE s_id='%s' ;",
