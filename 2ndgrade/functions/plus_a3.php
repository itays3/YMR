...
<body>
	...
	<?php 
	  ...
	  $the_answer = $x+$y;
	?>
<form method="post" action="plus_a3.php" dir="ltr">    
...
VALUES
('%s','1','$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[equal]$_POST[answer]','plus_a3','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[equal]$_POST[answer]','plus_a3','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='plus_a3'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET plus_a3='%s' WHERE s_id='%s' ;",
