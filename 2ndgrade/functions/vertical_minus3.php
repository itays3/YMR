...
<h1 dir="rtl" 
    style="font-family:Cursive;font-size:36px;color:#3300cc;font-style:italic;font-weight:normal;text-shadow:4px 4px 8px black;">חיסור במאונך - רמה קשה</h1>
	<p dir="rtl"
	...
	<?php 
	  ...
	  $the_answer = $x-$y;
?>		
<form method="post" action="vertical_minus3.php" dir="ltr">    
...
VALUES
('%s','1','$_POST[last_a]$_POST[minus]$_POST[last_b]$_POST[equal]$_POST[the_answer]','vertical_minus3','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[answer]$_POST[minus]$_POST[last_b]$_POST[equal]$_POST[oldanswer]','vertical_minus3','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
