...
<form method="post" action="plus_e1.php" dir="ltr">     
    ...
    <!-- this part shows the last exercise and the answer-->
		<tr>
			...
			<?php }?>
                <td><? include('answer1.php') ?></td></tr></table>
</form>
<?php 
session_start();
...
VALUES
('%s','1','$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_e1','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_e1','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='plus_e1'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET plus_e1='%s' WHERE s_id='%s' ;",
