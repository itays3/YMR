...
<body>
    ...
    <?php 
			...
			while ($x+$y+$z>1000)
			{
			 ...
			 $the_answer = $x+$y+$z;
		?>
    <h1 dir="rtl" style="font-family:Cursive;font-size:36px;color:#3300cc;font-style:italic;font-weight:normal;text-shadow:4px 4px 8px black;">תרגילי שרשרת עד 1000</h1>
...
<form method="post" action="plus_e3.php" dir="ltr">     
    ...
    <!-- this part shows the last exercise and the answer-->
		<tr>
			...
			<?php }?>
                <td><? include('answer3.php') ?></td></tr></table>
</form>
<?php 
session_start();
...
VALUES
('%s','1','$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_e3','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[plus]$_POST[last_y]$_POST[plus1]$_POST[last_z]$_POST[equal]$_POST[answer]','plus_e3','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='plus_e3'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET plus_e3='%s' WHERE s_id='%s' ;",
