...
<h1 dir="rtl" 
	...
	<?php 
	  ...
	  $the_answer = $x-$y;
?>
<form method="post" action="minus_b1.php" dir="ltr">    
...
VALUES
('%s','1','$_POST[last_x]$_POST[minus]$_POST[last_y]$_POST[equal]$_POST[answer]','minus_b1','%s','%s')", $row['id'],$grade,$grade_num);
...
'$_POST[last_x]$_POST[minus]$_POST[last_y]$_POST[equal]$_POST[answer]','minus_b1','%s','%s')", $row['id'],$grade,$grade_num);
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
  }
...
$query = "SELECT s_id, SUM(yes_count) FROM statistics WHERE s_id='$s_id' && subject='minus_b1'  GROUP BY s_id"; 
...
$query=sprintf("UPDATE connect SET minus_b1='%s' WHERE s_id='%s' ;",
